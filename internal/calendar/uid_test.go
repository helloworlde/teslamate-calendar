...
func TestDriveUIDStable(t *testing.T) {
	...
	e1 := DriveEvents("1", "Model 3", drives, "normal", true, time.UTC, "", "")
	e2 := DriveEvents("1", "Model 3", drives, "normal", true, time.UTC, "", "")
...
func TestChargeUIDStable(t *testing.T) {
	...
	e1 := ChargeEvents("1", "Model 3", charges, "normal", true, time.UTC, "", "")
	e2 := ChargeEvents("1", "Model 3", charges, "normal", true, time.UTC, "", "")
	if len(e1) != 1 || len(e2) != 1 || e1[0].UID != e2[0].UID {
